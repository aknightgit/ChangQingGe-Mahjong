...
export function selectDiscardTile(player: Player, game: GameState): string {
  ...
  for (let i = 0; i < discardCandidates.length; i++) {
    ...
    if (committedOpenSuit && isHonor(tile)) {
      ...
      // P0: 三口以上（≥3组露副）必须优先清光风牌，留自己一门冲向听牌
      // 不再奖励留风，改为惩罚：已露副越多越要扔风
      if (exposedMeldCount >= 3 && routeWantsCommittedSuit) {
        composite += 50 - exposedMeldCount * 8 + visibleCopies * 8
      } else if (tilePairCount <= 1 && exposedMeldCount >= 2 && routeWantsCommittedSuit) {
