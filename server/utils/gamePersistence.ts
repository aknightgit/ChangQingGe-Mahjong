...
const documentToGameState = (doc: PersistedMahjongGame): GameState => ({
  ...
  lastActionTime: doc.lastActionTime instanceof Date ? doc.lastActionTime.getTime() : Number(doc.lastActionTime),
  endedAt: doc.endedAt ? (doc.endedAt instanceof Date ? doc.endedAt.getTime() : Number(doc.endedAt)) : undefined,
  finalScores: doc.finalScores,
  roundStats: doc.roundStats || [],
  customScoringMode: (doc.customScoringMode ?? null) as GameState['customScoringMode'],
  pendingActions: doc.pendingActions ?? [],
  dice: doc.dice,
  diceRolls: doc.diceRolls,
