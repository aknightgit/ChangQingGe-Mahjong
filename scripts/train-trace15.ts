  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    for (const otherIdx of [nextPlayer, prevPlayer, oppositePlayer]) {
      ...
      if (canPeng(opp, discard)) {
          console.error(`PONG_CLAIM: ${opp.name} claims_pong hand=${opp.hand.length} melds=${opp.exposedMelds.length} drawn=${opp.hand.length} discard=${discard?.suit}-${discard?.value}`)
        ...
        if (Math.random() < pengChance) {
          ...
          console.error(`PONG_DONE: ${opp.name} hand_after_peng=${opp.hand.length} melds=${opp.exposedMelds.length} meldSources=${opp.meldSources.join(',')}`)
