    ...
    for (const otherIdx of [nextPlayer, prevPlayer, oppositePlayer]) {
      ...
      if (canPeng(opp, discard)) {
        ...
        if (Math.random() < pengChance) {
          ...
          console.error(`PONG_APPLY: ${opp.name} h=${opp.hand.length} melds=${opp.exposedMelds.length} base=${13 - 3 * opp.exposedMelds.length} before_base=${13 - 3 * (opp.exposedMelds.length - 1)}`)
