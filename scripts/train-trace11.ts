...
function aiDiscard(p: BotPlayer, gameMultiplier: number = 1, discardPile: Tile[] = [],
  ...
  console.error(`GAME: r=${round} discard ${p.name} h=${p.hand.length} disp=${tileStr(validTile)}`); return validTile
}
...
function runGame(akPolicy: BotPolicy, otherPolicies: BotPolicy[]): GameResult | null {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    turn = round
    const drawn = drawTile(g, player); if (player.name === 'AI-AK' && round >= 28 && round <= 36) { console.error(`GAME: r=${round} draw AK h=${player.hand.length} melds=${player.exposedMelds.length}`) }
    ...
    for (const otherIdx of [nextPlayer, prevPlayer, oppositePlayer]) {
      ...
      if (canPeng(opp, discard)) {
        ...
        if (Math.random() < pengChance) {
          ...
          applyPeng(opp, discard, curr); console.error(`GAME: r=${round} pong ${opp.name} h=${opp.hand.length} melds=${opp.exposedMelds.length}`)，失败则不push meld
