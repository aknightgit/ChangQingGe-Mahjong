/**
 * AI 竞技对战脚本 — 真实服务端逻辑版（ai-arena-real.ts）
 *
 * 与 ai-arena.ts 的区别：
 ...
 *
 * 用法：
 *   npx tsx scripts/ai-arena-real.ts                    # 默认 100 局
 *   npx tsx scripts/ai-arena-real.ts --games 5          # 指定局数
 *   npx tsx scripts/ai-arena-real.ts --no-detail        # 关闭逐局明细
 */
...
async function runRealGame(policies: any[], gameIdx: number = 0): Promise<GameResult | null> {
  ...
  const recordWinner = (p: BotPlayer, idx: number, isSelfDraw: boolean, wonFan: number, baseFan: number, roundNum: number, winningTile?: Tile, winningFrom?: string) => {
    ...
    const typeStrs = p.winHandType ? p.winHandType.split(',').filter(t => t.length > 0 && t !== 'standard') : []
...
function toGameRecord(gameId: number, seats: SeatAssignment[], result: GameResult | null, durationMs: number): GameRecord {
  if (!result) {
    ...
    return { gameId, seats, winner: -1, winnerAiName: null, scores: [0, 0, 0, 0], netScores: [0, 0, 0, 0], scoresByAi: scoresByAi as Record<AIName, number>, totalPot: 0, multiplier: 1, settlementCount: 0, isDraw: true, roundNum: 0, eventCount: 0, durationMs, settlements: [], events: [], snapshots: [], turnSnapshots: [] }
  }
  const scoresByAi: Record<string, number> = {}
...
function buildStats(records: GameRecord[]): Map<AIName, AiStats> {
  ...
  for (const rec of records) {
    ...
    if (rec.winnerAiName) { const st = stats.get(rec.winnerAiName)!; st.wins++; if (rec.selfDraw) st.selfDrawWins++; else st.discardWins++ }
  }
...
function formatSummary(stats: Map<AIName, AiStats>, records: GameRecord[]): string {
  ...
  for (const rec of records) { if (rec.isDraw || !rec.handTypeName) continue; for (const t of rec.handTypeName.split(',').map(s => s.trim()).filter(Boolean)) { handTypes[t] = (handTypes[t] || 0) + 1 } }
  const hte = Object.entries(handTypes).sort((a, b) => b[1] - a[1])
  if (hte.length > 0) {
    lines.push(`## 🀄 番种分布\n`); lines.push(`| 番种 | 次数 | 占比 |`); lines.push(`|------|------|------|`)
    for (const [name, count] of hte) { lines.push(`| ${name} | ${count} | ${(count / Math.max(1, winGames) * 100).toFixed(1)}% |`) }
    lines.push('')
  }
...
function formatGamesCsv(records: GameRecord[]): string {
  ...
  for (const rec of records) {
    ...
    lines.push([rec.gameId, s(0), s(1), s(2), s(3), winner, winType, rec.handTypeName || '-', rec.wonFan ?? '-', rec.multiplier, rec.roundNum, rec.totalPot, scores, rec.durationMs, rec.isDraw ? '1' : '0'].join(','))
  }
  return lines.join('\n')
}
