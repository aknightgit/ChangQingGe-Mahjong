    ...
    for (const otherIdx of [nextPlayer, prevPlayer, oppositePlayer]) {
      ...
      if (canPeng(opp, discard)) {
        ...
        if (Math.random() < pengChance) {
          ...
          meldTaken = true; if (opp.name === 'AI-AK') { console.error(`[TRACE_CLAIM] AK: after_claim hand=${opp.hand.length} melds=${opp.exposedMelds.length}`); }
          ...
          const pengDiscard = aiDiscard(opp, g.gameMultiplier, g.discardPile, g.wallIdx, g.deck.length, g.players, otherIdx); if (opp.name === 'AI-AK') { console.error(`[TRACE_CLAIM] AK: before_claim hand=${opp.hand.length} melds=${opp.exposedMelds.length}`); }
