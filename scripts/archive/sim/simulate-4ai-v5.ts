/**
 * 4AI 持续优化测试 v5
 * 10轮 × 500局，每轮后验证核心原则，修正后继续
 * 验证：胡牌逻辑、吃碰排斥、听牌最大化、百搭利益最大化
 */
...
function createBotPlayer(simPlayer: SimPlayer, index: number): any {
  return {
    id: `bot-${index}`,
    name: simPlayer.name,
    position: index,
...
function createBotGame(players: SimPlayer[], wall: Tile[], wallIdx: number, currentTurn: number): any {
  return {
    id: 'sim-game',
...
function validateRound(stats: any, round: number): string[] {
  const issues: string[] = [];
  
  // 1. 验证胡牌逻辑
  if (stats.winRate < 10) {
    issues.push(`[R${round}] 胡牌率过低 (${stats.winRate}%)，可能需要调整AI策略`);
  }
  
  // 2. 验证牌型分布
  const winTypes = stats.winTypes || {};
  const totalWins = stats.wins || 0;
  
  if (totalWins > 0) {
    // 检查是否有异常牌型
    for (const [type, count] of Object.entries(winTypes)) {
      const pct = ((count as number) / totalWins * 100).toFixed(1);
      if (pct === '100.0' && totalWins > 50) {
        issues.push(`[R${round}] 牌型单一：${type} 占 100% (${totalWins}次)`);
      }
    }
  }
  ...
  const winCounts = Object.values(playerWins) as number[];
  if (winCounts.length > 0) {
    const maxWin = Math.max(...winCounts);
    const minWin = Math.min(...winCounts);
    if (maxWin > minWin * 3 && totalWins > 20) {
      issues.push(`[R${round}] AI胜率不均衡：最高${maxWin} vs 最低${minWin}`);
    }
  }
...
async function main() {
  ...
  const gamesPerRound = parseInt(process.argv[3] || '500');
  
  console.log(`=== 4AI 持续优化测试 v5 ===`);
  console.log(` rounds=${rounds}, gamesPerRound=${gamesPerRound}\n`);
  
  const outputDir = path.join(__dirname, '../training-output/sim-4ai-v5');
  ...
  for (let round = 1; round <= rounds; round++) {
    ...
    stats.winRate = parseFloat(((stats.wins / stats.totalGames) * 100).toFixed(1));
    ...
    // 验证
    const issues = validateRound(stats, round);
    if (issues.length > 0) {
      console.log(`\n⚠️ 验证问题:`);
    ...
    // 输出
    const outputFile = path.join(outputDir, `round-${round}-sim-4ai-v5.json`);
    fs.writeFileSync(outputFile, JSON.stringify({ ...roundReport, timestamp: new Date().toISOString(), issues }, null, 2));
  ...
  const summaryFile = path.join(outputDir, 'summary-sim-4ai-v5.json');
