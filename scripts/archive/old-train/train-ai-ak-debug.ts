...
function drawTile(g: GameState, p: BotPlayer): Tile | null {
  ...
  p.hand.push(tile); if (process.env.DEBUG_HAND) console.error(`[HAND_DEBUG] draw: ${p.name} hand_size=${p.hand.length}`)
  // 诊断：追踪手牌，摸牌后手牌长度
...
function aiDiscard(p: BotPlayer, gameMultiplier: number = 1, discardPile: Tile[] = [],
  ...
  console.error(`[HAND_DEBUG] aiDiscard: ${p.name} hand_size=${p.hand.length} discarding=${tileStr(validTile)}`); return validTile
}
