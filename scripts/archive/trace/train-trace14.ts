...
function runGame(akPolicy: BotPolicy, otherPolicies: BotPolicy[]): GameResult | null {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    for (const otherIdx of [nextPlayer, prevPlayer, oppositePlayer]) {
      ...
      console.error(`CP_CHECK: ${opp.name} h=${opp.hand.length} norm_h=${normalizeHand(opp.hand).length} melds=${opp.exposedMelds.length} discard=${discard?.suit}-${discard?.value} canPeng=${canPeng(opp, discard)}`); if (canPeng(opp, discard)) {
