...
function aiDiscard(p: BotPlayer, gameMultiplier: number = 1, discardPile: Tile[] = [],
  ...
  console.error(`[T8] DISCARD: ${p.name} hand_before_dispose=${p.hand.length} disposing=${tileStr(validTile)}`); return validTile
}
...
function runGame(akPolicy: BotPolicy, otherPolicies: BotPolicy[]): GameResult | null {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    // T8 extra
    if (player.name === 'AI-AK') { console.error(`[T8] AFTER_DRAW hand=${player.hand.length} melds=${player.exposedMelds.length}`) }
