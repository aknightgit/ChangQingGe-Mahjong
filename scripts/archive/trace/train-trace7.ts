...
function runGame(akPolicy: BotPolicy, otherPolicies: BotPolicy[]): GameResult | null {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    turn = round
    const drawn = drawTile(g, player); if (player.name === 'AI-AK') { console.error(`[T7] ROUND_START: round=${round} curr=${curr} ${player.name} hand=${player.hand.length} melds=${player.exposedMelds.length} phase=draw_exp=~${14 - 3 * player.exposedMelds.length}`) }
