...
function runGame(akPolicy: BotPolicy, otherPolicies: BotPolicy[]): GameResult | null {
  ...
  if (opp.name === 'AI-AK') console.error(`[T3] AK: before_filter hand=${opp.hand.length} discard=${pengDiscard.suit}-${pengDiscard.value}(${pengDiscard.id})`)
  if (opp.name === 'AI-AK') console.error(`[T3] AK: after_filter hand=${opp.hand.length} melds=${opp.exposedMelds.length}`)
