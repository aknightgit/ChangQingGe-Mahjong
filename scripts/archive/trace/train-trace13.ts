  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    for (const otherIdx of [nextPlayer, prevPlayer, oppositePlayer]) {
      ...
      if (canPeng(opp, discard)) {
        ...
        if (Math.random() < pengChance) {
          ...
          console.error(`TRACE_PENG: ${opp.name} AFTER_PENG hand=${opp.hand.length} melds=${opp.exposedMelds.length}`)
          ...
          console.error(`TRACE_PONG_DISCARD: ${opp.name} AFTER_PONG_DISC hand=${opp.hand.length}`)
    ...
    if (canChow(nextP, discard) && Math.random() < nextP.policy.chowChance) {
      ...
      applyChow(nextP, discard, curr)  // 内部已normalize，失败则不push meld
          console.error(`TRACE_CHOW: ${nextP.name} AFTER_CHOW hand=${nextP.hand.length} melds=${nextP.exposedMelds.length}`)
      ...
      nextP.hand = nextP.hand.filter(t => t.id !== chowDiscard.id)
          console.error(`TRACE_CHOW_DISCARD: ${nextP.name} AFTER_CHOW_DISC hand=${nextP.hand.length}`)
