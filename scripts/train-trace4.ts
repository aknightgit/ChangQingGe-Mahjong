...
function applyPeng(p: BotPlayer, tile: Tile, sourcePos?: number): void {
  ...
  for (const u of matches) { const idx = p.hand.findIndex(rt => rt.id === u.id); if (idx >= 0) p.hand.splice(idx, 1); if (p.name === 'AI-AK') console.error(`[T4 applyPeng] ${p.name}: after_rem=${p.hand.length} idx=~${idx}`) }
