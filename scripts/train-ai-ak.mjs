 ...
 * 输出到 training-output/
 */
const tiles_1 = require("../server/utils/tiles");
const handValidator_1 = require("../server/utils/handValidator");
const scoring_1 = require("../server/utils/scoring");
const game_1 = require("../server/types/game");
const policyEngine_1 = require("../server/ai/pipeline/policyEngine");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const url_1 = require("url");
const training_reporter_1 = require("./training-reporter");
const __filename = import.meta.url || "train.mjs"
...
// ========== 全局错误处理（防止训练崩溃无日志） ==========
let _mainRoundReports = [];
let _mainBestPolicy = null;
let _mainMetrics = null;
let _mainMdFile = '';
    ...
    try {
        if (_mainMdFile && (_mainRoundReports.length > 0 || _mainBestPolicy)) {
            ...
            for (const report of _mainRoundReports) {
                if (report.round === 0 || report.round === ROUNDS + 1)
                    continue;
                mainOut.push((0, training_reporter_1.formatRoundReport)(report, false, `第${report.round}轮`));
            }
        ...
        console.error('[TRAIN_CRASH] 保存部分报告失败:', e);
    }
}
...
const HAND_TYPE_NAMES = {
    [handValidator_1.HandType.FENG_PENG]: '风碰', [handValidator_1.HandType.ALL_WIND]: '风一色', [handValidator_1.HandType.QING_PENG]: '清碰',
    [handValidator_1.HandType.HUN_PENG]: '混碰', [handValidator_1.HandType.EIGHT_FLOWERS]: '八花', [handValidator_1.HandType.FULL_FLUSH]: '清一色',
    [handValidator_1.HandType.HALF_FLUSH]: '混一色', [handValidator_1.HandType.FOUR_WILD]: '四百搭', [handValidator_1.HandType.ALL_TRIPLETS]: '碰碰胡',
    [handValidator_1.HandType.DA_DIAO]: '大吊'
};
...
function saveCharacter(name, policy, metrics) {
    const filePath = path.join(CHAR_DIR, `${name}.json`);
    const data = { savedAt: new Date().toISOString(), round: 0, metrics, policy };
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2), 'utf-8');
}
// ========== Mutate AI-AK policy ==========
function mutatePolicy(base, intensity = 1.0) {
    const mutated = { ...base };
    ...
    for (const key of keys) {
        ...
        if (!range) {
            console.error('[DEBUG] key not in PARAM_RANGES:', key);
            continue;
        }
        ...
        if (!Number.isFinite(current)) {
            console.error('[DEBUG] base[key] not finite:', key, '=', current);
            continue;
        }
        ...
        if (Math.random() < 0.15) {
            ...
            if (range) {
                ...
                if (Number.isFinite(rMin) && Number.isFinite(rMax)) {
                    ...
                    let noisy = Number(child[key]) + noise;
...
// ========== Tile helpers ==========
function t(suit, v, id) {
    return { suit, value: v, id: id || `${suit}-${v}-${Math.random().toString(36).slice(2, 8)}`, isFlower: false };
}
function tileEq(a, b) { if (!a || !b)
    return false; return a.suit === b.suit && a.value === b.value; }
// 数字→中文
const NUM_CN = ['', '一', '二', '三', '四', '五', '六', '七', '八', '九'];
// suit枚举值→中文（注意TileSuit枚举值是 'wan' 不是 'characters'，'tiao' 不是 'bamboos'）
const SUIT_CN = { dots: '筒', wan: '万', tiao: '条', feng: '风', jian: '箭', hua: '花' };
const WIND_CN = { 1: '东', 2: '南', 3: '西', 4: '北' };
const DRAGON_CN = { 1: '中', 2: '发', 3: '白' };
const FLOWER_CN = { 1: '春', 2: '夏', 3: '秋', 4: '冬', 5: '梅', 6: '兰', 7: '竹', 8: '菊' };
function tileStr(t) {
    if (!t)
        return '??';
    if (t.suit === game_1.TileSuit.FLOWER)
        return FLOWER_CN[t.value] || `花${t.value}`;
    if (t.suit === game_1.TileSuit.WIND)
        return WIND_CN[t.value] || '?';
    if (t.suit === game_1.TileSuit.DRAGON)
        return DRAGON_CN[t.value] || '?';
    return `${NUM_CN[t.value] || t.value}${SUIT_CN[t.suit] || t.suit}`;
}
function isHonor(t) { return t.suit === game_1.TileSuit.WIND || t.suit === game_1.TileSuit.DRAGON; }
function isWild(t, ws, wv) { return ws && wv ? t.suit === ws && t.value === wv : false; }
// ========== Config ==========
const AI_NAMES = ['AI-AK', 'AI-小胖', 'AI-阿水', 'AI-老赵'];
const AK_IDX = 0;
function buildDeck() {
    const d = [];
    for (const s of [game_1.TileSuit.DOTS, game_1.TileSuit.CHARACTERS, game_1.TileSuit.BAMBOOS])
        for (let v = 1; v <= 9; v++)
            for (let c = 0; c < 4; c++)
                d.push(t(s, v));
    for (let v = 1; v <= 4; v++)
        for (let c = 0; c < 4; c++)
            d.push(t(game_1.TileSuit.WIND, v));
    for (let v = 1; v <= 3; v++)
        for (let c = 0; c < 4; c++)
            d.push(t(game_1.TileSuit.DRAGON, v));
    for (let i = 0; i < 8; i++)
        d.push({ suit: game_1.TileSuit.FLOWER, value: i + 1, id: `f${i}`, isFlower: true });
    return (0, tiles_1.shuffleTiles)(d);
}
function setupGame(akPolicy, otherPolicies, meta) {
    const deck = buildDeck();
    const nonFlower = deck.filter(t => !(0, tiles_1.isFlower)(t));
    const w = nonFlower[Math.floor(Math.random() * nonFlower.length)];
    const ws = w.suit, wv = w.value;
    const policies = [akPolicy, otherPolicies[0], otherPolicies[1], otherPolicies[2]];
    const players = AI_NAMES.map((name, i) => ({
        name, pos: i, hand: [], exposedMelds: [], flowerTiles: [],
        isBot: true, isTing: false, score: 0, wildSuit: ws, wildValue: wv, kongCount: 0, id: `p${i}`,
        status: 'playing', policy: policies[i],
        meldSources: [0, 0, 0, 0], discardedTiles: [],
...
function drawTile(g, p) {
    if (g.wallIdx >= g.deck.length)
        return null;
    const tile = g.deck[g.wallIdx++];
    if (!tile)
        return drawTile(g, p);
    if ((0, tiles_1.isFlower)(tile)) {
    ...
    if (process.env.DEBUG_DISCARD === '1')
        console.error(`[DISCARD_RET] ${p.name} returns=${tileStr(tile)} hand_before=${p.hand.length}`);
    return tile;
}
function isWT(t, p) { return isWild(t, p.wildSuit, p.wildValue); }
function makeWT(p) { return p.wildSuit && p.wildValue ? `${p.wildSuit}-${p.wildValue}` : null; }
// ========== Meld detection ==========
...
function canChow(p, tile) {
    if (!tile || isHonor(tile) || tile.suit === game_1.TileSuit.FLOWER)
        return false;
    const v = tile.value;
    // 三种吃牌方式：
    ...
    const has = (val) => normalized.some(t => t.suit === tile.suit && t.value === val);
    // 中间牌
    if (v >= 2 && v <= 8 && has(v - 1) && has(v + 1))
        return true;
    // 最低牌：tile是被吃序列中最大的
    if (v >= 3 && has(v - 1) && has(v - 2))
        return true;
    // 最高牌：tile是被吃序列中最小的
...
function checkHandInvariant(p, phase) {
    ...
    if (len !== expected) {
        ...
        if (phase === 'draw' || phase === 'discard')
            console.error(`[铁律详情] ${p.name} ${phase} rawHandLen=${p.hand.length} handTiles=[${actualHand}]`);
        p._lastPhase = phase;
        p._lastHand = len;
        return false;
    }
...
// ========== Apply melds ==========
function applyPeng(p, tile, sourcePos) {
    ...
    for (const m of matches) {
        ...
        if (idx >= 0) {
            removedIds.push(p.hand[idx].id.slice(-6));
            p.hand.splice(idx, 1);
        }
        else {
            console.error(`[PENG_BUG] ${p.name} match tile not found in hand! id=${m.id.slice(-6)}`);
        }
    }
    ...
    p.exposedMelds.push({ type: game_1.MeldType.TRIPLET, tiles: [tile, tile, tile], isConcealed: false });
    if (sourcePos !== undefined && sourcePos !== p.pos)
        p.meldSources[sourcePos]++;
}
function applyChow(p, tile, sourcePos) {
    p.hand = normalizeHand(p.hand); // 铁律：apply前先normalize
    const before = p.hand.length;
    const meldCount = p.exposedMelds.length;
    const validBefore = before === 13 - 3 * meldCount; // K哥铁律：只看口数
    const v = tile.value;
    const findTile = (suit, val) => p.hand.find(t => t.suit === suit && t.value === val);
    // 用 tileEq（suit+value）移除，绕过 ID 重复 bug
    const removeTile = (t) => { const idx = p.hand.findIndex(h => tileEq(h, t)); if (idx >= 0)
        p.hand.splice(idx, 1); };
    // 三种吃牌模式，与canChow一致
    let t1, t2;
    ...
    if (t1.id === t2.id) {
        console.error(`BUG applyChow: same tile! ${p.name} tile=${tileStr(tile)} t1=t2=${t1.id}`);
        return;
    }
    removeTile(t1);
    removeTile(t2);
    const after = p.hand.length;
    if (after !== before - 2)
        console.error(`BUG applyChow: ${p.name} before=${before} after=${after} (expected ${before - 2}) tile=${tileStr(tile)} t1=${t1.id} t2=${t2.id}`);
    // 排序tiles为从小到大
    const meldTiles = [t1, tile, t2].sort((a, b) => a.value - b.value);
    p.exposedMelds.push({ type: game_1.MeldType.SEQUENCE, tiles: meldTiles, isConcealed: false });
    if (sourcePos !== undefined && sourcePos !== p.pos)
        p.meldSources[sourcePos]++;
}
function applyMingKong(p, tile, sourcePos) {
    ...
    const after = p.hand.length;
    if (after !== before - 3) {
        console.error(`BUG applyMingKong: ${p.name} before=${before} after=${after}`);
        return;
    }
    p.exposedMelds.push({ type: game_1.MeldType.KONG, tiles: [tile, tile, tile, tile], isConcealed: false });
    p.kongCount++;
    if (sourcePos !== undefined && sourcePos !== p.pos)
        p.meldSources[sourcePos]++;
}
function applyAnKong(p, tile) {
    p.hand = normalizeHand(p.hand); // 铁律：先normalize
    const tileCount = p.hand.filter(t => tileEq(t, tile)).length;
    if (tileCount < 4) {
    ...
    const after = p.hand.length;
    if (after !== before - 4) {
        console.error(`BUG applyAnKong: ${p.name} before=${before} after=${after} (expected ${before - 4})`);
        return;
    }
    p.exposedMelds.push({ type: game_1.MeldType.CONCEALED_KONG, tiles: [tile, tile, tile, tile], isConcealed: true });
    p.kongCount++;
}
function applyJiaGang(p, tile) {
    p.hand = normalizeHand(p.hand); // 铁律：先normalize
    const tileCount = p.hand.filter(t => tileEq(t, tile)).length;
    if (tileCount < 1) {
    ...
    const meld = p.exposedMelds.find(m => m.type === game_1.MeldType.TRIPLET && tileEq(m.tiles[0], tile));
    if (!meld)
        return;
    meld.type = game_1.MeldType.KONG;
    ...
    const after = p.hand.length;
    if (after !== before - 1) {
        console.error(`BUG applyJiaGang: ${p.name} before=${before} after=${after}`);
        return;
    }
    p.kongCount++;
}
...
function rollMultiplier() {
    ...
    if (isBigPair)
        return 4; // 1+1 or 4+4 = ×4
    if (isPair)
        return 2; // other doubles = ×2
    return 1;
}
// 模拟全局倍数（流局/造反继承）
let prevRoundWasDraw = false;
function nextGameMultiplier(meta) {
    const d1 = Math.floor(Math.random() * 6) + 1;
    const d2 = Math.floor(Math.random() * 6) + 1;
...
function calcScore(p, isSelfDraw, isKongWin, gameMultiplier) {
    const wildTileId = p.wildSuit && p.wildValue ? `${p.wildSuit}-${p.wildValue}` : null;
    const types = (0, handValidator_1.detectHandTypes)(p.hand, p.exposedMelds, wildTileId, isSelfDraw, p.flowerTiles.length);
    const result = (0, scoring_1.calculateScore)({
...
function applyBaoSettlement(g, winnerIdx, isSelfDraw, discarderIdx, baseScore, mult = 1) {
...
//             碰碰胡=公式, 混一色=公式
function evalWildDeployment(hand, meldCount, wildCount, flowerCount, globalMult = 1) {
    ...
    const calcExtra = (hasWild, menqing) => {
        let m = 1;
        if (!hasWild)
            m *= 2; // 无百搭×2
        if (menqing)
    ...
    const typeBaseScore = {
        [handValidator_1.HandType.FENG_PENG]: 40, // 风碰=40（最高固定番）
        [handValidator_1.HandType.ALL_WIND]: 20, // 风一色=20
        [handValidator_1.HandType.QING_PENG]: 20, // 清碰=20
        [handValidator_1.HandType.FULL_FLUSH]: 10, // 清一色=10
        [handValidator_1.HandType.HUN_PENG]: 10, // 混碰=10
        [handValidator_1.HandType.ALL_TRIPLETS]: 10, // 碰碰胡=10（公式上限）
        [handValidator_1.HandType.HALF_FLUSH]: 10, // 混一色=10（公式上限）
        [handValidator_1.HandType.FOUR_WILD]: 10, // 四百搭=10
        [handValidator_1.HandType.DA_DIAO]: 10, // 大吊=10
        [handValidator_1.HandType.EIGHT_FLOWERS]: 10, // 八花=10
    };
    if (wildCount === 0) {
        // 无百搭时：手牌本身即无百搭，肯定有×2
        const types = (0, handValidator_1.detectHandTypes)(hand, [], null, false, flowerCount);
    ...
    const typesNoWild = (0, handValidator_1.detectHandTypes)(nonWild, [], null, false, flowerCount);
    ...
    let flushScore = 0;
    if (meldCount === 0) {
        for (const suit of [game_1.TileSuit.DOTS, game_1.TileSuit.CHARACTERS, game_1.TileSuit.BAMBOOS]) {
            const suitTiles = nonWild.filter(t => t.suit === suit);
            if (suitTiles.length + wildCount >= 13) {
                flushScore = 10;
                break;
            }
        }
    ...
    let fengPengScore = 0;
    if (honorCount + wildCount >= 13)
        fengPengScore = 40; // 风碰=40
    // 评估4：百搭做碰碰胡（公式：2 + 花数 + 面子分，上限10）
    const groups = (0, tiles_1.groupTiles)(nonWild);
    let pairPotential = 0;
...
// ========== AI Discard (长清阁规则) ==========
function aiDiscard(p, gameMultiplier = 1, discardPile = [], wallIdx = 0, deckLen = 144, allPlayers = [], myPos = 0, turnNumber = 0) {
    // 铁律：hand可能含undefined，在计算前先normalize
    p.hand = normalizeHand(p.hand);
    const policy = p.policy;
    const hand = p.hand;
    if (process.env.DEBUG_DISCARD === '1')
        console.error(`[DISCARD] ${p.name} hand_before=${p.hand.length} pos=${p.pos}`);
    ...
    const suits = [game_1.TileSuit.DOTS, game_1.TileSuit.CHARACTERS, game_1.TileSuit.BAMBOOS];
    ...
    const oppDiscardBySuit = { [game_1.TileSuit.DOTS]: 0, [game_1.TileSuit.CHARACTERS]: 0, [game_1.TileSuit.BAMBOOS]: 0 };
    // 取最近的出牌（约前20张 = 前5轮）
    const recentDiscards = discardPile.slice(-Math.min(20, discardPile.length));
    for (const t of recentDiscards) {
        if (t.suit === game_1.TileSuit.DOTS || t.suit === game_1.TileSuit.CHARACTERS || t.suit === game_1.TileSuit.BAMBOOS) {
            oppDiscardBySuit[t.suit]++;
        }
    }
    // 对手都不要的花色 → 我做这一门更容易
    const discardObsBoost = {};
    ...
    if (allPlayers.length > 0) {
        for (let i = 0; i < allPlayers.length; i++) {
            ...
            // 对手副露多（3+）→ 可能快听
            if (opp.exposedMelds.length >= 3)
                oppTingSignal += 0.3;
            // 对手出过幺九 → 可能在清理 → 可能已听
            const termDiscards = opp.discardedTiles.filter(t => !isHonor(t) && (t.value === 1 || t.value === 9)).length;
    ...
    if (isHighMult && honorCount >= 5)
        mHHo = policy.multHighHonorStart;
    // ====== 积分榜动态策略 ======
    ...
    for (const tile of hand) {
        ...
        const suitAnalysis = suits.map((s, suitIdx) => {
            ...
            const usefulTiles = suitTiles.filter(t => {
                ...
                if (tileCount >= 3)
                    return true; // 刻子（碰/杠）
                if (tileCount === 2)
                    return true; // 对子（进张可碰/可碰）← 修复：漏算对子
                const others = suitTiles.filter(o => !tileEq(o, t));
                if (others.some(o => o.value === t.value))
                    return true;
        ...
        // ---- 4. 选择最优目标路线（按潜力分数量化） ----
        let targetRoute = 'normal';
        let targetRouteScore = 0;
        if (canPureFlush) {
            const s = longestSuit.count * 5 - longestSuit.isolatedCount * 3 + 100;
            if (s > targetRouteScore) {
                targetRouteScore = s;
                targetRoute = 'pure';
            }
        }
        if (canHalfFlush) {
            const s = longestSuit.count * 4 + honorCount * 3 + (pairCount >= 1 ? 20 : 0) + 80;
            if (s > targetRouteScore) {
                targetRouteScore = s;
                targetRoute = 'half';
            }
        ...
        if (canAllPungs) {
            const s = pengHuGroups * 30 + pairCount * 10 + 60;
            if (s > targetRouteScore) {
                targetRouteScore = s;
                targetRoute = 'pungs';
            }
        ...
        // 最短门孤立张 → 必须打
        if (forceDiscardShortest && isInShortestSuit && isIsolated)
            keepScore -= 50;
        // 次短门孤立张 → 必须打（做一色时）
        if (forceDiscardSecond && isInSecondSuit && isIsolated)
            keepScore -= 20;
        // 做一色时，次短门对子也要拆
        if (forceDiscardSecond && isInSecondSuit && count >= 2 && forceBreakPairs)
            keepScore -= 35;
        // 做一色时，最短门对子也要拆
        if (forceDiscardShortest && isInShortestSuit && count >= 2 && forceBreakPairs)
            keepScore -= 45;
        // 无明确路线时，最短门孤立张也适度惩罚
        if (targetRoute === 'normal' && isInShortestSuit && isIsolated)
            keepScore -= 20;
        // 目标门加分（做某门时，该门所有牌都加分）
        if (isInTargetSuit)
            keepScore += 15;
        // 非目标门孤立张适度惩罚
        if (!isInTargetSuit && isIsolated && !isInShortestSuit && !isInSecondSuit)
            keepScore -= 8;
        // ====== K哥时序决策层（综合方案：出牌轮数 + 牌墙剩余） ======
        // deckLen - wallIdx = 牌墙剩余张数（约144张开始，摸一张少一张）
        const wallRemaining = deckLen - wallIdx;
        p._discardTurns = (p._discardTurns ?? 0) + 1;
        const myTurns = p._discardTurns;
        // Phase 1（< 3次出牌 OR 牌墙≥75张）：机械式拆散牌，不管路线
        if (myTurns < 3 || wallRemaining >= 75) {
            if (isInShortestSuit && isIsolated)
                keepScore -= 80;
            if (isInSecondSuit && isIsolated)
                keepScore -= 50;
            if (isInShortestSuit && count >= 2)
                keepScore -= 30;
            if (isInSecondSuit && count >= 2)
                keepScore -= 15;
        }
        // Phase 2（3-6次出牌 AND 50≤牌墙<75）：抉择方向阶段
            ...
            if (!p._chosenRoute) {
                ...
                if (best === pureScore)
                    p._chosenRoute = 'pure';
                else if (best === pungsScore)
                    p._chosenRoute = 'pungs';
                else
                    p._chosenRoute = 'honors';
            }
            const route = p._chosenRoute;
            if (route === 'pure' && !isInTargetSuit && isIsolated)
                keepScore -= 30;
            if (route === 'pungs' && count >= 2)
                keepScore += 5;
            if (route === 'honors' && !isHonor && isIsolated)
                keepScore -= 30;
        }
        // Phase 3（≥6次出牌 OR 牌墙<50张）：按路线弃牌
            ...
            else if (p._chosenRoute === 'pungs') {
                if (count >= 2)
                    keepScore += 15;
                if (count === 1)
                    keepScore -= 20;
            }
            else if (p._chosenRoute === 'pure' || p._chosenRoute === 'half') {
                if (!isInTargetSuit && isIsolated)
                    keepScore -= 30;
                if (count >= 2 && !isInTargetSuit)
                    keepScore -= 15;
            }
            else if (p._chosenRoute === 'honors') {
                if (!isHonor && isIsolated)
                    keepScore -= 30;
                if (!isHonor && count >= 2)
                    keepScore -= 15;
            }
        ...
        // ====== 原有评分逻辑保留 ======
        // 碰碰胡保留对子
        if (targetRoute === 'pungs' && count >= 2)
            keepScore += 10;
        if (count >= 2)
            keepScore += policy.pairWeight;
        if (count >= 3)
            keepScore += policy.tripletKeepBonus;
        if (count >= 4)
            keepScore += policy.tripletKeepBonus * 2;
        if (!isHonor(tile) && tile.suit !== game_1.TileSuit.FLOWER) {
        ...
        if ((tile.value === 1 || tile.value === 9) && !isHonor(tile)) {
            ...
            if (neighbors.length === 0)
                keepScore -= policy.terminalPenalty;
        }
        if (tile.suit === game_1.TileSuit.WIND) {
            let wk = policy.windGeneralKeep;
            if (tile.value === 1)
                wk += policy.windEastKeep;
            else if (tile.value === 2)
                wk += policy.windSouthKeep;
            else if (tile.value === 3)
                wk += policy.windWestKeep;
            else if (tile.value === 4)
                wk += policy.windNorthKeep;
            if (count >= 2)
                keepScore += wk * policy.pairWeight;
            if (count >= 3)
                keepScore += wk * 3;
            if (count >= 4)
                keepScore += wk * 5;
            if (count === 1)
                keepScore -= wk * 0.5;
            if (isAllHonorsRoute)
                keepScore += policy.allHonorsPursuit * 10 * (count >= 2 ? 2 : 1);
            if (isAllHonorsRoute && isAllPungsRoute)
                keepScore += policy.allHonorsPungsPursuit * 20;
        }
        if (tile.suit === game_1.TileSuit.DRAGON) {
            let dk = policy.dragonGeneralKeep;
            if (tile.value === 1)
                dk += policy.dragonRedKeep;
            else if (tile.value === 2)
                dk += policy.dragonGreenKeep;
            else if (tile.value === 3)
                dk += policy.dragonWhiteKeep;
            if (count >= 2)
                keepScore += dk * policy.pairWeight;
            if (count >= 3)
                keepScore += dk * 4;
            if (count >= 4)
                keepScore += dk * 6;
        ...
        if (policy.pureFlushPursuit > 0 && !isHonor(tile)) {
            if (tile.suit === suits[maxSuitIdx])
                ...
                keepScore -= policy.pureFlushPursuit * 2;
        }
        // ====== 对手出牌观察：对手都不要的花色 → 我做这一门更容易 ======
        if (!isHonor(tile)) {
            const obsBoost = discardObsBoost[tile.suit] || 0;
            if (obsBoost > 0)
                keepScore += obsBoost * 5;
            // 如果我正在做的花色就是对手都不要的，额外加分
            if (tile.suit === suits[maxSuitIdx] && obsBoost > 0)
                keepScore += obsBoost * 3;
        }
        // 倍数×起手牌质量联合加成
        ...
        if (mHHo > 0 && isHonor(tile))
            keepScore += mHHo * 5 * (count >= 2 ? 2 : 1);
        ...
        if (policy.allPungsPursuit > 0 && isAllPungsRoute) {
            if (count >= 2)
                keepScore += policy.allPungsPursuit * 5;
        ...
        if (wildIsOptimal && wildEval) {
            ...
            if (wildEval.bestType === '风碰' && isHonor(tile))
                keepScore += 8; // 保留风/箭牌
            if (wildEval.bestType === '清一色' && !isHonor(tile) && tile.suit === suits[maxSuitIdx])
                keepScore += 6;
            if (wildEval.bestType === '碰碰胡' && count >= 2)
                keepScore += 5;
        }
        ...
        if (wildCount >= 1 && tilesNeeded <= 3 && policy.wildDiaoKeepBonus > 0) {
            ...
            if (isWT(tile, p))
                keepScore += policy.wildDiaoKeepBonus * 5; // 百搭绝不打
            // 碰碰胡路线 + 百搭大吊
            if (count >= 2 && policy.wildDiaoPungBoost > 0)
                keepScore += policy.wildDiaoPungBoost * 3;
        ...
        if (wildCount === 0) {
            // 无百搭×2翻倍 → 碰碰胡更快成型更有价值
            if (count >= 2)
                keepScore += policy.allPungsPursuit * 3;
            // 更低的门清意愿：无百搭已经×2了，门清额外×2收益相对变小，不如快成型
            if (isMenqing)
                keepScore -= policy.menqingKeepBonus * 0.5;
            // 速度优先：快听牌收无百搭翻倍
            if (!isHonor(tile) && count === 1) {
                const neighbors = sameSuit.filter(t => Math.abs(t.value - tile.value) <= 2);
        ...
        if (isHighMult && hasGoodHand) {
            ...
            if (count >= 2)
                keepScore += policy.multHighValueBias * 4;
            if (!isHonor(tile) && tile.suit === suits[maxSuitIdx]) {
                keepScore += policy.multHighValueBias * 3 * (maxSuitCount / hand.length);
            }
            // 门清保持：清碰/清一色需要门清翻倍
            if (isMenqing && wildCount >= 1)
                keepScore += policy.menqingKeepBonus * 0.5;
            // 风箭也想留（冲风一色/风碰）
            if (isHonor(tile) && count >= 2)
                keepScore += policy.multHighValueBias * 2;
        }
        ...
        if (isWT(tile, p))
            keepScore += policy.wildKeepPenalty;
        // ====== 互包追踪：同家快触发包三 → 慎重吃碰 ======
        ...
        if (wallPhase === 'early') {
            ...
            if (count === 1 && !isHonor(tile))
                keepScore += policy.wallLateDefense * 2; // 打出孤立牌
        }
        // ====== 对手听牌时的安全牌优先 ======
        if (oppLikelyTing && policy.safeTilePriority > 0) {
            ...
            if (inDiscardPile)
                keepScore += policy.safeTilePriority * 5; // 已出过的牌很安全
            else
                keepScore -= policy.safeTilePriority * 2; // 未出过的牌有风险
        }
        // ====== 积分榜动态调整 ======
        if (isLoser && policy.scoreBehindRiskBoost > 0) {
            // 落后→冒险！百搭更值钱，大牌更值得冲
            if (isWT(tile, p))
                keepScore += policy.scoreBehindRiskBoost * 3; // 百搭绝对保留
            if (count >= 2)
                keepScore += policy.scoreBehindRiskBoost * 2; // 保留对子做碰碰胡
            if (!isHonor(tile) && tile.suit === suits[maxSuitIdx])
                keepScore += policy.scoreBehindRiskBoost * 1.5; // 保留主花色做清一色
        }
        if (isBigLeader && policy.scoreLeadDefenseBoost > 0) {
            // 大幅领先→防守！降低大牌追求，快胡收分
            if (count <= 1 && !isHonor(tile))
                keepScore += policy.scoreLeadDefenseBoost * 2; // 打孤立牌
    ...
    if (!validTile) {
        ...
        const allTiles = Object.values(game_1.TileSuit).flatMap(s => s === game_1.TileSuit.FLOWER ? [] : Array.from({ length: 9 }, (_, i) => ({ suit: s, value: i + 1, id: `fallback-${s}-${i + 1}` })));
        return allTiles[0];
    }
    return validTile;
}
...
function normalizeHand(hand) {
    return hand.filter(t => t && !(0, tiles_1.isFlower)(t));
}
// ========== 血战到最后一人 ==========
// 每局有人胡牌后，已胡玩家退出，剩余玩家继续打，直到最后1人
function runGameWithFightToLast(akPolicy, otherPolicies) {
...
function runGame(akPolicy, otherPolicies, gameIdx = 0) {
    const runGameStart = performance.now();
    // 每局开始时清空 isTing 缓存（不同局wild牌不同）
    (0, handValidator_1.clearIsTingCache)();
    (0, handValidator_1.clearCanWinCache)();
    const gameStart = performance.now();
    const gameMeta = {};
    const g = setupGame(akPolicy, otherPolicies, gameMeta);
    const events = [];
    const settlementLog = [];
    const winnersThisGame = []; // 追踪本局所有赢家（血战到底）
    const finishedPlayers = new Set(); // 已胡牌退出的玩家（血战）
    let turn = 0;
    // 每回合快照（--detail 时收集）
    const turnSnapshots = [];
    // buildResult: 血战模式统一出口，构造 GameResult
    const buildResult = (primaryWinner, winMode, baseScore, handType, fanScore, discarder) => {
    ...
    const recordPayment = (from, to, amount, reason, fan, mult) => {
        settlementLog.push({ from, to, amount, reason, fan, mult });
    };
    // 记录赢家到winnersThisGame（每个winner return前调用）
    // 格式化面子：按花色分组，同花色内刻→顺→杠，用分号分隔
    const formatMelds = (melds, wildCount) => {
        if (melds.length === 0)
            return [];
        // 排序：先花色，再类型（碰>顺>杠）
        const sorted = [...melds].sort((a, b) => {
            if (a.tiles[0].suit !== b.tiles[0].suit)
                return a.tiles[0].suit.localeCompare(b.tiles[0].suit);
            const order = (t) => t.type === game_1.MeldType.TRIPLET ? 0 : t.type === game_1.MeldType.SEQUENCE ? 1 : 2;
        ...
        for (const m of sorted) {
            ...
            if (suit !== currentSuit || m.type !== currentType) {
                if (groupTiles.length > 0 && currentSuit !== null) {
                    const typeStr = currentType === game_1.MeldType.TRIPLET ? '碰' : currentType === game_1.MeldType.SEQUENCE ? '顺' : '杠';
    ...
    const recordWinner = (p, idx, isSelfDraw, wonFan, baseFan, roundNum, winningTile) => {
        // 手牌分组：按花色分组，普通牌在前，百搭在后并加(*)
        // 重要：完整手牌 = 当前手牌 + 所有副露面子里的牌（与 recordSnapshots 一致）
        const wildSuit = p.wildSuit, wildVal = p.wildValue;
        const isWT2 = (t) => wildSuit && wildVal ? t.suit === wildSuit && t.value === wildVal : false;
        ...
        const filteredTiles = (isSelfDraw && winningTile)
            ...
            : allTiles;
        const normalTiles = filteredTiles.filter(t => !(0, tiles_1.isFlower)(t) && !isWT2(t));
        const wildTiles = filteredTiles.filter(t => !(0, tiles_1.isFlower)(t) && isWT2(t));
        const suitGroups = [];
        for (const suit of ['Wan', 'Tong', 'Tiao']) {
            const normal = normalTiles.filter(t => t.suit === suit);
            const wild = wildTiles.filter(t => t.suit === suit);
            const parts = [...normal.map(t => tileStr(t)), ...wild.map(t => tileStr(t) + '(*)')];
            if (parts.length > 0)
                suitGroups.push(parts.join(' '));
        }
        const handStr = suitGroups.join(' ; ');
        // 门口牌分组（百搭不在这里重复显示，手牌里已标(*)）
        const meldStrs = formatMelds(p.exposedMelds, wildTiles.length);
        // 【新增】计算牌型中文名（用于玩家明细显示）
        const typeStrs = p.winHandType ? p.winHandType.split(',').filter(t => t.length > 0 && t !== handValidator_1.HandType.STANDARD) : [];
        const typeNames = typeStrs.map(t => HAND_TYPE_NAMES[t] || t).filter(t => t);
    ...
    const recordSnapshots = () => {
        return g.players.map(p => {
            ...
            const wildTileName = wildTileStr ? tileStr({ suit: wildSuit, value: wildVal, id: '' }) : '(无百搭)';
            ...
            return {
                ...
                melds: p.exposedMelds.map(m => `${m.type === game_1.MeldType.TRIPLET ? '碰' : m.type === game_1.MeldType.SEQUENCE ? '吃' : m.type === game_1.MeldType.KONG ? '杠' : '?'}:${m.tiles.map(t => tileStr(t)).join(' ')}`),
                flowers: p.flowerTiles.map(t => tileStr(t)),
    ...
    const log = (player, action, detail) => { events.push({ turn, player, action, detail }); };
    // 每回合快照：记录当前玩家摸打，供每圈明细使用
    // drawnTile 和 discardedTile 显式传入：表示当前玩家本回合的摸打
    const recordTurnSnapshot = (curr, drawnTile, discardedTile) => {
        ...
        turnSnapshots.push({
            ...
            players: g.players.map(p => ({
                name: p.name,
                hand: (0, tiles_1.sortTiles)([...p.hand]).map(t => {
                    const base = tileStr(t);
                    return isWT(t, p) ? base + '*' : base;
                }).join(' '),
                exposed: p.exposedMelds.map(m => `${m.type === game_1.MeldType.TRIPLET ? '碰' : m.type === game_1.MeldType.SEQUENCE ? '吃' : m.type === game_1.MeldType.KONG ? '明杠' : m.type === game_1.MeldType.CONCEALED_KONG ? '暗杠' : '?'}:${(0, tiles_1.sortTiles)([...m.tiles]).map(t => tileStr(t)).join(' ')}`),
                meldSources: [...p.meldSources],
                handCount: p.hand.length,
                flowers: p.flowerTiles.map(t => tileStr(t))
            })),
            wildTile: g.wildSuit && g.wildValue ? tileStr({ suit: g.wildSuit, value: g.wildValue, id: '' }) : '无百搭',
    ...
    for (let round = 0; round < MAX_ROUNDS; round++) {
        ...
        if ((0, tiles_1.isFlower)(drawn)) {
            log(player.name, '补花', tileStr(drawn));
            recordTurnSnapshot(curr, tileStr(drawn), '-');
            continue;
        }
        ...
        // [DEBUG] 追踪canWin诊断
        const numPungs = player.exposedMelds.filter(m => m.type === game_1.MeldType.TRIPLET || m.type === game_1.MeldType.SEQUENCE).length;
        const winCheck = (0, handValidator_1.canWin)(normalizedHand, player.exposedMelds, makeWT(player));
        ...
        if (winCheck.canWin) {
            ...
            if (Math.random() < winChance) {
                ...
                for (let i = 0; i < 4; i++) {
                    if (i !== curr)
                        g.players[i].score -= baseScore;
                }
                // 互包结算
                applyBaoSettlement(g, curr, true, null, baseScore, 1);
                for (let i = 0; i < 4; i++) {
                    if (i !== curr)
                        recordPayment(g.players[i].name, player.name, baseScore, '自摸', baseFan, g.gameMultiplier);
                }
        ...
        for (const ak of canAnKong(player)) {
            if (Math.random() < player.policy.anKongChance) {
                ...
                if (extra && !(0, tiles_1.isFlower)(extra)) {
                    if ((0, handValidator_1.canWin)(normalizeHand(player.hand), player.exposedMelds, makeWT(player)).canWin) {
                        ...
                        for (let i = 0; i < 4; i++) {
                            if (i !== curr)
                                recordPayment(g.players[i].name, player.name, baseScore * g.gameMultiplier, '杠上自摸', baseFan, g.gameMultiplier);
                        }
        ...
        player.isTing = (0, handValidator_1.isTing)(player.hand, player.exposedMelds.length, makeWT(player));
        ...
        if (player.exposedMelds.length >= 2) {
            const cw = (0, handValidator_1.canWin)(normalizeHand(player.hand), player.exposedMelds, makeWT(player));
        ...
        for (let other = 0; other < 4; other++) {
            ...
            if ((0, handValidator_1.canWin)(testHand, opp.exposedMelds, makeWT(opp), false).canWin) {
        ...
        for (const otherIdx of [nextPlayer, prevPlayer, oppositePlayer]) {
            ...
            if (canMingKong(opp, discard)) {
                if (Math.random() < opp.policy.kongChance) {
                    ...
                    if (!extra) {
                        ...
                        return { winner: -1, scores: g.players.map(p => p.score), events, multiplier: g.gameMultiplier, settlementLog, snapshots: [], roundNum: turn, winnersThisGame: [], turnSnapshots };
                    }
                    checkHandInvariant(opp, 'draw'); // 杠后摸牌
                    if (extra && !(0, tiles_1.isFlower)(extra)) {
                        if ((0, handValidator_1.canWin)(normalizeHand(opp.hand), opp.exposedMelds, makeWT(opp)).canWin) {
                            ...
                            for (let i = 0; i < 4; i++) {
                                if (i !== otherIdx)
                                    g.players[i].score -= kongBaseScore;
                            }
                            applyBaoSettlement(g, otherIdx, true, null, kongBaseScore, 1);
                            for (let i = 0; i < 4; i++) {
                                if (i !== otherIdx)
                                    recordPayment(g.players[i].name, opp.name, kongBaseScore, '明杠自摸', baseFan, g.gameMultiplier);
                            }
                    ...
                    recordTurnSnapshot(otherIdx, extra && !(0, tiles_1.isFlower)(extra) ? tileStr(extra) : '-', tileStr(kongDiscard));
                    g.current = (otherIdx + 1) % 4;
                    meldTaken = true;
                    break;
                }
            ...
            if (canP) {
                ...
                if (!(0, handValidator_1.checkChowPongExclusion)(opp.chowPongExclusion, 'pong', discard.suit))
                    continue;
                let shouldPeng = false;
                if (opp.name === 'AI-AK' && REWARD_MODE) {
                    // AI-AK: 用 pipeline scorer 决定是否碰
                    try {
                        const ctx = (0, policyEngine_1.buildActionContext)(g, opp.id, ['PENG', 'PASS'], round * 4 + otherIdx);
                        const ranked = (0, policyEngine_1.rankActions)(ctx);
                ...
                if (shouldPeng) {
                    ...
                    if (opp.exposedMelds.length === meldCountBefore)
                        continue; // apply失败，跳过pong（不设置meldTaken）
                    meldTaken = true;
                    ...
                    opp.chowPongExclusion = (0, handValidator_1.updateChowPongExclusion)(opp.chowPongExclusion, 'pong', discard.suit); // K哥铁律：记录碰行动
                    ...
                    for (const ak of canAnKong(opp)) {
                        ...
                        if (extra && !(0, tiles_1.isFlower)(extra)) {
                            if ((0, handValidator_1.canWin)(normalizeHand(opp.hand), opp.exposedMelds, makeWT(opp)).canWin) {
                                ...
                                for (let i = 0; i < 4; i++) {
                                    if (i !== otherIdx)
                                        recordPayment(g.players[i].name, opp.name, kongBaseScore, '碰杠后自摸', baseFan, g.gameMultiplier);
                                }
        ...
        const chowExcluded = !(0, handValidator_1.checkChowPongExclusion)(nextP.chowPongExclusion, 'chow', discard.suit);
        if (nextP.name === 'AI-AK' && REWARD_MODE && canChow(nextP, discard)) {
            if (chowExcluded) {
                ...
                try {
                    const ctx = (0, policyEngine_1.buildActionContext)(g, nextP.id, ['CHOW', 'PASS'], round * 4 + nextPlayer);
                    const ranked = (0, policyEngine_1.rankActions)(ctx);
        ...
        if (shouldChow) {
            if (!(0, handValidator_1.checkChowPongExclusion)(nextP.chowPongExclusion, 'chow', discard.suit))
                continue; // K哥铁律：吃碰排斥
            const beforeChowMelds = nextP.exposedMelds.length;
            applyChow(nextP, discard, curr); // 内部已normalize，失败则不push meld
            if (nextP.exposedMelds.length === beforeChowMelds)
                continue; // apply失败，跳过chow
            nextP.chowPongExclusion = (0, handValidator_1.updateChowPongExclusion)(nextP.chowPongExclusion, 'chow', discard.suit); // K哥铁律：记录吃行动
            checkHandInvariant(nextP, 'claim'); // 吃后（未出牌）铁律
            // 放炮胡检查（claim后draw前，手牌=expectedLen）
            const handAfterChow = normalizeHand(nextP.hand);
            if ((0, handValidator_1.canWin)(handAfterChow, nextP.exposedMelds, makeWT(nextP), false).canWin) {
            }
            checkHandInvariant(nextP, 'draw'); // 吃后加杠仍可摸牌（杠不在禁止范围内）
            if (extra && !(0, tiles_1.isFlower)(extra)) {
                if ((0, handValidator_1.canWin)(normalizeHand(nextP.hand), nextP.exposedMelds, makeWT(nextP)).canWin) {
                    ...
                    for (let i = 0; i < 4; i++) {
                        if (i !== nextPlayer)
                            recordPayment(g.players[i].name, nextP.name, kongBaseScore, '吃杠后自摸', baseFan, g.gameMultiplier);
                    }
...
function formatRoundMarkdown(roundNo, evalResult, bestPolicy) {
...
function evaluatePolicy(akPolicy, otherPolicies, games) {
    const scores = {};
    const wins = {};
    ...
    for (let g = 0; g < games; g++) {
        ...
        if (result) {
            ...
            if (akDelta > 0 && (!bigWin || akDelta > bigWin.score))
                bigWin = { gameIdx: g, result, score: akDelta };
            if (akDelta < 0 && (!bigLoss || akDelta < bigLoss.score))
                bigLoss = { gameIdx: g, result, score: akDelta };
            ...
            if (winnerCount > 0) {
                if (winnerCount >= 2)
                    fightToLastGames++;
                if (winnerCount >= 1)
                    multiWinDist[Math.min(winnerCount, 4) - 1]++;
                ...
                for (const w of gameWinners) {
                    ...
                    const typeNamesRaw = typeStrs.filter(t => t !== handValidator_1.HandType.STANDARD).map(t => HAND_TYPE_NAMES[t] || t);
                    const typeNames = typeNamesRaw.length > 0 ? typeNamesRaw : ['普通']; // '普通' 仅作 display fallback
                    const bigTypes = [handValidator_1.HandType.FENG_PENG, handValidator_1.HandType.ALL_WIND, handValidator_1.HandType.QING_PENG];
                    if (typeStrs.some(t => bigTypes.includes(t)))
                        bigWinGames++;
                    if (w.melds.length === 0)
                        menqingWinGames++;
                    for (const t of typeNames)
                        handTypeDist[t] = (handTypeDist[t] || 0) + 1;
    ...
    // 大牌率（目标3-8%）
    if (bigHandRate < 0.03)
        mf -= (0.03 - bigHandRate) * 500;
    if (bigHandRate > 0.08)
        mf -= (bigHandRate - 0.08) * 500;
    // 门清胡牌率（目标7-12%）
    if (menqingWinRate < 0.07)
        mf -= (0.07 - menqingWinRate) * 400;
    if (menqingWinRate > 0.12)
        mf -= (menqingWinRate - 0.12) * 400;
    return {
...
function main() {
    try {
        ...
        // 重置 isTing 缓存统计
        (0, handValidator_1.resetIsTingCacheStats)();
        ...
        const scoreHistory = [baseline.akScore];
        let plateauCount = 0;
        let lastRoundEval = null;
        const roundReports = [];
        // Round 0 baseline: 仅 ROUNDS>1 或 GAMES_PER_ROUND>1 时才记录（避免 1×1 时多余）
        if (ROUNDS > 1 || GAMES_PER_ROUND > 1) {
            const baselineReport = (0, training_reporter_1.buildRoundReport)(0, baseline, bestPolicy, AI_NAMES, 'train-ai-ak.ts');
        ...
        for (let round = 1; round <= ROUNDS; round++) {
            ...
            let intensity = 1.0;
            if (plateauCount >= 2)
                intensity = 1.8; // more aggressive when stuck
            if (plateauCount >= 4)
                intensity = 2.5; // very aggressive
            ...
            let roundBigWin = null;
            let roundBigLoss = null;
            let roundWorstLoss = null;
            const roundLines = [];
            ...
            if (bestEvalResult) {
                ...
                const summaryLine = `  Current standings: ` + AI_NAMES.map(n => `${n}:${bestEvalResult.scores[n]}(${(bestEvalResult.winRates[n] * 100).toFixed(0)}%)`).join('  ');
            ...
            if (DETAIL_MODE && bestEvalResult) {
                const report = (0, training_reporter_1.buildRoundReport)(round, bestEvalResult, roundBestPolicy, AI_NAMES, 'train-ai-ak.ts');
                roundReports.push(report);
                const filename = (0, training_reporter_1.writeRoundFile)(OUT_DIR, report, true); // showDetail=true
        ...
        // Print cache stats
        const tingStats = (0, handValidator_1.getIsTingCacheStats)();
        const canWinStats = (0, handValidator_1.getCanWinCacheStats)();
        ...
        if (ROUNDS > 1 || GAMES_PER_ROUND > 1) {
            ...
            const finalReport = (0, training_reporter_1.buildRoundReport)(ROUNDS + 1, finalEval, bestPolicy, AI_NAMES, 'train-ai-ak.ts');
            const finalReportFormatted = (0, training_reporter_1.formatRoundReport)(finalReport, false, '最终评估');
        ...
        const mainOut = [...logLines];
        for (const report of roundReports) {
            if (report.round === 0 || report.round === ROUNDS + 1)
                continue; // 跳过初始评估和最终评估
            mainOut.push((0, training_reporter_1.formatRoundReport)(report, false, `第${report.round}轮`));
        }
        ...
        const indexFile = (0, training_reporter_1.writeIndexFile)(OUT_DIR, roundReports);
        console.log(`\nLog saved: ${mdFile}`);
