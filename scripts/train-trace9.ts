...
function aiDiscard(p: BotPlayer, gameMultiplier: number = 1, discardPile: Tile[] = [],
  ...
  process.stdout.write(`[T9 DISC] ${p.name} hand_before=${p.hand.length} discarding=${tileStr(validTile)}\n`); return validTile
}
...
function runGame(akPolicy: BotPolicy, otherPolicies: BotPolicy[]): GameResult | null {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    turn = round
    const drawn = drawTile(g, player); if (player.name === 'AI-AK') { process.stdout.write(`[T9 DRAW] r=${round} curr=${curr} ${player.name} hand=${player.hand.length} melds=${player.exposedMelds.length}\n`) }
