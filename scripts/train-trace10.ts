...
function aiDiscard(p: BotPlayer, gameMultiplier: number = 1, discardPile: Tile[] = [],
  ...
  if (round >= 30 && round <= 35) { process.stdout.write(`[T10 DISC] r=${round} ${p.name} hand=${p.hand.length} discarding=${tileStr(validTile)}\n`) }; return validTile
}
...
function runGame(akPolicy: BotPolicy, otherPolicies: BotPolicy[]): GameResult | null {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    turn = round
    const drawn = drawTile(g, player); if (player.name === 'AI-AK' && round >= 30 && round <= 35) { process.stdout.write(`[T10 DRAW] r=${round} ${player.name} hand=${player.hand.length} melds=${player.exposedMelds.length}\n`) }
