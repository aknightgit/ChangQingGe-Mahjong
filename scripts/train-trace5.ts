  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    for (const otherIdx of [nextPlayer, prevPlayer, oppositePlayer]) {
      ...
      if (opp.exposedMelds.length >= 4) continue  // 最多4组牌
          if (opp.name === 'AI-AK') { console.error(`[T5] canPeng_check: ${opp.name} hand=${opp.hand.length} melds=${opp.exposedMelds.length} discard=${discard.suit}-${discard.value}`) }
      if (canPeng(opp, discard)) {
        ...
        if (Math.random() < pengChance) {
          ...
          applyPeng(opp, discard, curr); if (opp.name === 'AI-AK') { console.error(`[T5] after_applyPeng: ${opp.name} hand=${opp.hand.length} melds=${opp.exposedMelds.length}`) }，失败则不push meld
          ...
          opp.hand = opp.hand.filter(t => t.id !== pengDiscard.id); if (opp.name === 'AI-AK') { console.error(`[T5] after_discard_filter: ${opp.name} hand=${opp.hand.length}`) }
